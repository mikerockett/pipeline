...
it('implements pipeline contract', function () {
...
it('is immutable when piping', function () {
  $pipeline = new Pipeline;
  $withPipe = $pipeline->pipe(fn (): int => 1);
  expect($pipeline)->not->toBe($withPipe);
});

it('pipes operations', function () {
  $result = (new Pipeline)
    ->pipe(fn (): int => 1)
    ->process('1');
...
it('processes payloads', function () {
  $result = (new Pipeline)
...
it('processes payloads in sequence', function () {
  $result = (new Pipeline)
...
it('processes payloads using stage contract implementations', function () {
  $result = (new Pipeline)
